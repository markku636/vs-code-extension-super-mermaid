...
```powershell
cd d:\project\integration\super-mermaid
npm install                # 第一次需要
...
```powershell
code --install-extension .\super-mermaid-0.4.0.vsix
```

或在 VS Code 內:Extensions 面板 → 右上角 `...` 選單 → **Install from VSIX...** → 選取 .vsix 檔。

裝完後 **Reload Window**(命令面板 → `Developer: Reload Window`)。
...
>
> 確認是否已安裝:`code --list-extensions | Select-String mermaid`,看到 `mark-ku.super-mermaid` 即成功。

> 更新版本時:改完程式碼 → 建議把 `package.json` 的 `version` 加一號 → 重跑 `npm run package` 與 `code --install-extension`。只跑 `npm run build` 不會更新已安裝的副本。
